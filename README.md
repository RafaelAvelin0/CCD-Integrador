...
* **Equipe:** * Clenylson Perycles de Oliveira
    * José Bezerra Filho
...
- [x] **Sensoriamento Básico:** Leitura contínua de temperatura e umidade (AM2301).
...
- [ ] **Integração Home Assistant:** Estabelecer comunicação (via MQTT) para centralização dos dados e controle do painel principal.
- [ ] **Aprimoramento de Segurança:** Avaliar implementação de handshakes seguros ou credenciais encriptadas para comunicação de rede.
...
* **Monitoramento Ambiental de Precisão:** Leitura contínua de temperatura e umidade via sensor AM2301 (DHT21).
...
```bash
git clone [https://github.com/RafaelAvelin0/CCD-Integrador.git](https://github.com/RafaelAvelin0/CCD-Integrador.git)
```
...
💻 Acesso ao Console Remoto (Auditoria)
---------------------------------------

Para visualizar os _logs_ e eventos em tempo real do datacenter sem deslocação ao local, abra um terminal em qualquer computador na mesma rede e conecte-se via Telnet ou Netcat:
```bash
# Exemplo (substitua pelo IP estático configurado no secrets.h)
nc 192.168.1.50 23
```
>_Nota: O console TCP permite apenas uma conexão por vez para otimizar o uso da memória RAM do ESP-12F._
